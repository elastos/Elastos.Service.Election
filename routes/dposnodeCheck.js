...
const didResolver = require("./../constant/systemConfig").COOIX_DPOS_NODE_SERVER_DID_RESOLVER;
const didAuth = require("./../constant/systemConfig").COOIX_DPOS_NODE_SERVER_DID_AUTH;
...
router.post('/jwtsave', async function (req, res, next) {
    try {
        ...
        let body = JSON.stringify({ method: "resolvedid", params: { did, all: false } });
        let headers = { "Content-Type": "application/json", "Authorization": didAuth };
        let result = await fetch(didResolver, {method: 'post', body, headers});
        let json = await result.json();
        let payload = json.result.transaction[0].operation.payload;
        let pks = JSON.parse(String(Buffer.from(payload, 'base64'))).publicKey;
        let pkh;
        for (value of pks) {
            if (value.id === "#primary") {
                pkh = bs58.decode(value.publicKeyBase58).toString("hex");
                break;
            }
        }
        let kp = ec.keyFromPublic(pkh, "hex").getPublic();
        let x = Buffer.from(kp.getX().toString(16), "hex").toString("base64");
        let y = Buffer.from(kp.getY().toString(16), "hex").toString("base64");
        let jwk = JWK.asKey({crv: "P-256", kty: "EC", algo: "ES256", use: "sig", x, y});
        JWT.verify(jwt, jwk);

        var sql = "INSERT INTO didtoken (`json`, `token`) VALUES ('" + did + "', '" + jwt + "')";
